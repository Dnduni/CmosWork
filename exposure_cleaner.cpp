...
int askfor_res();
int askfor_var();
int askfor_ctrls(ASI_CAMERA_INFO infoi);
int set_ctrls(int setvaluectrl);
int set_res(int setvalueres);
ASI_CAMERA_INFO init_prog();



int main(){
	ofstream stream; //opens file stream
	//Step 1,2,3
    ASI_CAMERA_INFO info = init_prog();
    ASI_ERROR_CODE except;

	//Step 4 Get number and type of controls and list them
    int response,Ncontrols = askfor_ctrls(info);
...
ASI_CAMERA_INFO init_prog(){
	...
	ASI_CAMERA_INFO infoi; //Struct containing camera info
	...
	ASIGetCameraProperty(&infoi, 0); //Writes relevant camera information to the info vector via pointer
	std::cout << "Camera id:" << infoi.CameraID<<"\n" << "Max image Height:" << infoi.MaxHeight <<"\t" << "Max image Width:" << infoi.MaxWidth << std::endl ;

	ASI_ERROR_CODE except; //Checks for errors when opening and initialising camera, will warn user if problems are detected
	except = ASIOpenCamera(infoi.CameraID); //Try Open
	except = ASIInitCamera(infoi.CameraID); //Try init
	if (except != 0 ){ //Warn
		std::cout << "Problem with connected camera, check connection" << std::endl;
	}
	else{ //Success

		std::cout << "Camera connection successful. \t Connected camera:" << infoi.CameraID << std::endl;
	}
...
int askfor_ctrls(ASI_CAMERA_INFO infoi){
    ...
    int iNcontrols;
	ASIGetNumOfControls(infoi.CameraID, &iNcontrols); //Get number
	std::cout << "Number of available controls:" << iNcontrols << std::endl; //Print control number
	std::cout << "Would you like to list them? (0/1)" << std::endl; //Ask for list
	int iresponse;
	std::cin >> iresponse; //Wait for response
	std::cout << std::endl;
    return iresponse,iNcontrols;
}